...
本项目仅供学习、研究和个人数据处理实验使用，不得用于商业用途，不得用于批量分发、转售、镜像数据库或绕过平台授权限制。
禁止将抓取的数据打包、转售或用于任何商业数据库。
NutriData 数据及接口归其权利方所有。
使用者应自行确认账号权限、使用条款、数据授权范围和当地法律法规要求，开发者不承担任何因使用本工具而产生的后果
